...
@ComponentConfigs({
    @ComponentConfig(lifecycle = UIFormLifecycle.class, template = "app:/templates/UISharedFile.gtmpl", events = {
        @EventConfig(listeners = SharedFileUIActivity.ViewDocumentActionListener.class),
...
public class SharedFileUIActivity extends BaseUIActivity{

  private static final String NEW_DATE_FORMAT = "hh:mm:ss MMM d, yyyy";

  private static final Log   LOG               = ExoLogger.getLogger(SharedFileUIActivity.class);
  ...
  public String getDocumentSummary(Map<String, String> activityParams) {
    return activityParams.get(SharedFileUIActivity.DOCUMENT_SUMMARY);
  }
  ...
  public void setUIActivityData(Map<String, String> activityParams) {
    this.contentLink = activityParams.get(SharedFileUIActivity.CONTENT_LINK);
    this.nodeUUID = activityParams.get(SharedFileUIActivity.ID);
    this.state = activityParams.get(SharedFileUIActivity.STATE);
    this.author = activityParams.get(SharedFileUIActivity.AUTHOR);
    this.dateCreated = activityParams.get(SharedFileUIActivity.DATE_CREATED);
    this.lastModified = activityParams.get(SharedFileUIActivity.LAST_MODIFIED);
    this.contentName = activityParams.get(SharedFileUIActivity.CONTENT_NAME);
    this.message = activityParams.get(SharedFileUIActivity.MESSAGE);
    this.mimeType = activityParams.get(SharedFileUIActivity.MIME_TYPE);
    this.imagePath = activityParams.get(SharedFileUIActivity.IMAGE_PATH);
    this.docTypeName = activityParams.get(SharedFileUIActivity.DOCUMENT_TYPE_LABEL);
    this.docTitle = activityParams.get(SharedFileUIActivity.DOCUMENT_TITLE);  
    this.docVersion = activityParams.get(SharedFileUIActivity.DOCUMENT_VERSION);
    this.docSummary = activityParams.get(SharedFileUIActivity.DOCUMENT_SUMMARY);
    this.activityStatus = activityParams.get(SharedFileUIActivity.ACTIVITY_STATUS);
  }
  ...
  public String[] getSystemCommentBundle(Map<String, String> activityParams) {
    ...
    String tmp = activityParams.get(SharedFileUIActivity.IS_SYSTEM_COMMENT);
    String commentMessage;
    if (tmp==null) return null;
    try {
      if (Boolean.parseBoolean(tmp)) {
        commentMessage  = activityParams.get(SharedFileUIActivity.MESSAGE);
  ...
  public String[] getSystemCommentTitle(Map<String, String> activityParams) {
    String[] result;
    if (activityParams==null) return null;
    String commentValue = activityParams.get(SharedFileUIActivity.SYSTEM_COMMENT);
  ...
  public static class ViewDocumentActionListener extends EventListener<SharedFileUIActivity> {
    @Override
    public void execute(Event<SharedFileUIActivity> event) throws Exception {
      final SharedFileUIActivity docActivity = event.getSource();
  ...
  public static class DownloadDocumentActionListener extends EventListener<SharedFileUIActivity> {
    @Override
    public void execute(Event<SharedFileUIActivity> event) throws Exception {
    	SharedFileUIActivity uiComp = event.getSource() ;
