...
@ComponentConfig(lifecycle = UIFormLifecycle.class, template = "app:/templates/UISharedContent.gtmpl", events = {
    @EventConfig(listeners = SharedContentUIActivity.ViewDocumentActionListener.class),
...
public class SharedContentUIActivity extends BaseUIActivity {

  private static final String NEW_DATE_FORMAT = "hh:mm:ss MMM d, yyyy";

  private static final Log   LOG               = ExoLogger.getLogger(SharedContentUIActivity.class);
  ...
  public String getDocumentSummary(Map<String, String> activityParams) {
    return activityParams.get(SharedContentUIActivity.DOCUMENT_SUMMARY);
  }
  ...
  public void setUIActivityData(Map<String, String> activityParams) {
    this.contentLink = activityParams.get(SharedContentUIActivity.CONTENT_LINK);
    this.nodeUUID = activityParams.get(SharedContentUIActivity.ID);
    this.state = activityParams.get(SharedContentUIActivity.STATE);
    this.author = activityParams.get(SharedContentUIActivity.AUTHOR);
    this.dateCreated = activityParams.get(SharedContentUIActivity.DATE_CREATED);
    this.lastModified = activityParams.get(SharedContentUIActivity.LAST_MODIFIED);
    this.contentName = activityParams.get(SharedContentUIActivity.CONTENT_NAME);
    this.message = activityParams.get(SharedContentUIActivity.MESSAGE);
    this.mimeType = activityParams.get(SharedContentUIActivity.MIME_TYPE);
    this.imagePath = activityParams.get(SharedContentUIActivity.IMAGE_PATH);
    this.docTypeName = activityParams.get(SharedContentUIActivity.DOCUMENT_TYPE_LABEL);
    this.docTitle = activityParams.get(SharedContentUIActivity.DOCUMENT_TITLE);
    this.docVersion = activityParams.get(SharedContentUIActivity.DOCUMENT_VERSION);
    this.docSummary = activityParams.get(SharedContentUIActivity.DOCUMENT_SUMMARY);
  }
  ...
  public String[] getSystemCommentBundle(Map<String, String> activityParams) {
    ...
    String tmp = activityParams.get(SharedContentUIActivity.IS_SYSTEM_COMMENT);
    String commentMessage;
    if (tmp==null) return null;
    try {
      if (Boolean.parseBoolean(tmp)) {
        commentMessage  = activityParams.get(SharedContentUIActivity.MESSAGE);
  ...
  public String[] getSystemCommentTitle(Map<String, String> activityParams) {
    String[] result;
    if (activityParams==null) return null;
    String commentValue = activityParams.get(SharedContentUIActivity.SYSTEM_COMMENT);
  ...
  public static class ViewDocumentActionListener extends EventListener<SharedContentUIActivity> {
    @Override
    public void execute(Event<SharedContentUIActivity> event) throws Exception {
      final SharedContentUIActivity docActivity = event.getSource();
